...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Random-block coarse-grain of hemibrain edge list; outputs aggregated edge CSV."
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=100,
        help="Target nodes per block (deterministic assignment; default 100).",
    )
    parser.add_argument(
        "--seed",
        type=int,
        default=0,
        help="RNG seed for block assignment (default 0).",
    )
    parser.add_argument(
        "--output-prefix",
        default="outputs/coarse_random_blocks",
...
def assign_blocks(count: int, block_size: int, seed: int) -> tuple[np.ndarray, int]:
    """
    Deterministically assign nodes to random blocks of size block_size.
    ...
    for start in range(0, count, block_size):
        ...
        block_ids[perm[start:end]] = block_id
        block_id += 1
...
def main() -> int:
    ...
    n = A.shape[0]
    block_ids, num_blocks = assign_blocks(n, args.block_size, args.seed)
