...
This module instantiates the abstract MaleCNS benchmark protocol, same-trial
provenance graphs, and consumer evidence policies with concrete artifact
specifications, download locations, dataset versions, and SHA-256 verification.
"""
...
ROOT_GAUTHEY_DATASET = ProvenanceRoot("dataset:gauthey:2026", RootKind.DATASET, GAUTHEY_WHOLE_BRAIN_SOURCE.stable_identifier)
...
CANONICAL_ARTIFACT_SPECS: dict[str, ArtifactSpec] = {
    # Tier 1: Connectome Graph & Annotations (~560 MB)
    ...
    "body_annotations": ArtifactSpec(
        ...
        description="MaleCNS curated neuron annotations (classes, types, hemilineages, sex dimorphism)",
    ),
    ...
    # Tier 2: Functional Imaging (~150 MB - 1.5 GB depending on scope)
    "functional_trial_calcium": ArtifactSpec(
        key="functional_trial_calcium",
        role="functional_observation",
        tier="functional",
        relative_path="functional/gauthey_trial01_dff_rois.csv",
        download_url="https://doi.org/10.5281/zenodo.17618684",
        expected_size_bytes=45_000_000,
        source=GAUTHEY_WHOLE_BRAIN_SOURCE,
        dataset_version="gauthey-lightbead-2026",
        provenance_roots=(
            ROOT_GAUTHEY_DATASET,
            ROOT_GAUTHEY_ANIMAL,
            ROOT_GAUTHEY_TRIAL,
            ROOT_GAUTHEY_OPTICAL_ACQ,
            ROOT_GAUTHEY_PREPROCESSING,
        ),
        description="Extracted dF/F whole-brain functional timeseries across standard ROIs",
    ),

    # Tier 3: Registration Maps (~35 MB)
    "registration_bifrost_map": ArtifactSpec(
        ...
        description="BIFROST coordinate and neuropil alignment between functional volumes and connectome",
    ),

    # Tier 4: Motor & Effector Architecture (~10 MB)
    "motor_neuron_muscle_map": ArtifactSpec(
        ...
        description="VNC motor neuron to leg and wing muscle targeting correspondences",
    ),

    # Tier 5: Behavioural Kinematics (~25 MB)
...
class MaleCNSManifest:
    """Executable experiment manifest for MaleCNS embodied structure-function benchmark."""

    def __init__(
        self,
        base_dir: str | Path = "data/malecns",
    ...
    def total_expected_bytes(self, tiers: Iterable[str] | None = None) -> int:
        ...
        return sum(
            ...
            if selected_tiers is None or spec.tier in selected_tiers
        )

    def build_provenance_graph(self) -> ProvenanceGraph[str]:
        mapping = {
            spec.key: frozenset(spec.provenance_roots)
            for spec in self.specs.values()
        }
        return ProvenanceGraph(mapping)

    def create_artifact_receipt(
        self, key: str, sha256_digest: str | None = None
    ) -> ArtifactReceipt:
        spec = self.specs[key]
        path = self.target_path(key)
    ...
    def create_benchmark_split_receipt(
        ...
        split_canonical = f"{split_artifact_name}\n{train_hash}\n{held_out_hash}".encode("utf-8")
        from hashlib import sha256
        split_hash = sha256(split_canonical).hexdigest()

        return BenchmarkSplitReceipt(
            split_artifact_sha256=split_hash,
            train_ids_sha256=train_hash,
            held_out_ids_sha256=held_out_hash,
    ...
    def build_consumer_bundles(
        ...
        """Construct typed ConsumerEvidenceBundles indexed by Animalexic policy."""
        return {
            EvidenceConsumer.STRUCTURE_FUNCTION: ConsumerEvidenceBundle(
                policy=STRUCTURE_FUNCTION_POLICY,
                channels=frozenset({
                    EvidenceChannel.CONNECTOME,
                    EvidenceChannel.CALCIUM,
                    EvidenceChannel.REGISTRATION,
                }),
            ...
            EvidenceConsumer.EFFECTOR_STATE: ConsumerEvidenceBundle(
                ...
                channels=frozenset({
                    EvidenceChannel.MUSCLE_ACTIVATION,
                    EvidenceChannel.KINEMATICS,
                }),
                dependence_relation=dependence_relation,
                provenance_adequate=provenance_adequate,
            ),
            ...
            EvidenceConsumer.SEMANTIC: ConsumerEvidenceBundle(
                ...
                channels=frozenset({
                    EvidenceChannel.KINEMATICS,
                    EvidenceChannel.INTERACTION,
                    EvidenceChannel.ENVIRONMENT,
                }),
                dependence_relation=dependence_relation,
                provenance_adequate=provenance_adequate,
            ),
